...
description: >-
  ...
  Do NOT trigger for: simple code explanations (use /explain), debugging
  (use /flow), or static documentation (use /readme).
...
Write to: `{project}/.claude/livebrief/{timestamp}-{slug}.html`

- `{timestamp}`: ISO date (`2026-04-05`)
- `{slug}`: kebab-case summary of the topic (max 40 chars)
- Create the directory if it doesn't exist
- Add `.claude/livebrief/` to `.gitignore` if not already there
...
4. **Persist decisions** — write a summary to `{project}/.claude/livebrief/{same-slug}-decisions.json`
