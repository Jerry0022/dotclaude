...
| Keepalive pulser stopped (`PULSER_EXIT`) | Page shows "nicht verbunden" despite server running | Send a manual `curl -s -X POST /heartbeat`, then re-launch the pulser (bridge-server.md § step 3, task 1). `reason=DUPLICATE_PULSER` needs nothing — an older pulser has the port |
| Pickup waker stopped without a submission (`WAKER_EXIT reason=SERVER_DEAD`) | Submissions go unnoticed while the indicator may still be green | The bridge has not answered for ≥ 5 min. Restart it ON THE SAME PORT (a new one orphans the state file, the open tab, and makes fresh watchers exit PORT_CHANGED), then re-launch the waker; the pulser reconnects on its own |
| Page says "getrennt" for a few seconds after coming back to the tab or after a PC wake | The verdict was taken from a sample that predates the freeze | Fixed in pages generated with the freeze-aware checker (gate 3d). An older page needs its § Claude Connection Heartbeat block re-synced from templates.md |
...
| User reports comments missing | "my notes are gone" after a tab switch, a reload, or a round Claude never answered | **Never tell the user to retype.** `GET /draft?slug={slug}` — `recovered` holds the last non-empty value of every note the page ever autosaved, and it is fsynced to disk, so it survives a browser wipe and a power cut. Read them back to the user (or restore them into the page) before anything else. Then find out which of gate 49–53 the page is missing — a page generated before those entries carries the defect until its engine block is re-synced |
| Draft mirror unreachable | Page shows the `.draft-offline-strip` ("notes are saved in this browser only") | The bridge is down or answering 507 while the user keeps typing. The text is safe locally for now but has no second copy — restart the bridge on the SAME port (a new port means a new store path and the strip stays up), then confirm `GET /draft?slug={slug}` reports the notes |
| Strip says "bridge unreachable" / line says "getrennt" while `curl /heartbeat` answers 200 and `claude_ts` is fresh | `drafts/<slug>.jsonl` stopped growing ~27 autosaves into the session; `POST /draft` from curl still works | A page on the pre-gate-65 engine: its autosave posts with `keepalive` and never reads the answer, and Chromium refuses every keepalive request once 64 KiB of unread `no-store` responses are booked. Nothing reaches the wire — the bridge never sees the requests. Immediate relief: the user reloads the tab (a new document gets a fresh quota; the notes are in localStorage and merge back). Lasting fix: re-sync the page's § State Persistence block from templates.md on the next append (gate 65), or restart the bridge on the SAME port with a server that no longer sends `no-store` on POST answers |
