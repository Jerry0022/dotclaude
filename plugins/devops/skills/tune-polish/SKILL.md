...
2. **From `/run-agents`** — pass `--invoked-by=agents` plus
...
1. **State-visuals gaps** — same as `/tune-harden` Step 4 #3:
...
1. **Confirm UI causation** — for each backend finding, verify it
   ...
   is outside scope, defer (`/tune-harden` territory).
...
`skills/concept/deep-knowledge/templates.md` (with the submit/monitor
bridge from `skills/concept/deep-knowledge/bridge-server.md` — reference
