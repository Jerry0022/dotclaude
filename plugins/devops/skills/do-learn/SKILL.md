...
description: >-
  ...
  "/do-learn", "devops learn" (also misspelt — "devos learn",
...
invokes: [auto-issue]
triggers:
  en: ["/do-learn", "devops learn", "devos learn", "/devops-learn", "remember this for the project", "capture learning"]
...
Text passed after `/do-learn` is the learning — with two shapes that need
...
immediately before `/do-learn`; go further back only while the newer
...
is not the trigger: *"in jedem Projekt soll `/do-ship` die Issue-Nummer
schreiben"* changes a plugin default and is a plugin rule (Q1). Nor does naming
a plugin part keep the gate shut — *"sprich mich per Du an, auch wenn `/do-ship`
nachfragt"* is a tone preference that merely mentions `/do-ship` as the venue;
...
- **the devops plugin** — a plugin skill (`/do-ship`, `/auto-fix`, `/auto-concept`, …),
...
|------------------------------------------------------------------------------------|----------------------------------------------|
| "`/do-ship` bricht ab, wenn der Branchname einen Slash enthält" (consumer project)     | **B** — plugin defect, upstream issue        |
...
| "In jedem Projekt soll `/do-ship` die Issue-Nummer schreiben" (consumer project)        | **B** — plugin default, not the gate         |
| "`/do-ship` soll hier den Docker-Publish überspringen, wir bauen kein Image" (either)   | **C-override** — reason holds off upstream   |
