...
belongs to `run-autonomous` (its no-ship engine). A `weiter` nudge from the
...
2. **Live concept decisions** — for every `needs-decision` / `oversized` item,
   **now** generate a `/concept` decision page and let the user decide the
...
Reuse the `run-autonomous` frame by calling its **shared scripts** and
referencing its deep-knowledge — do NOT duplicate that prose here.

1. **Permission audit + priming** — run `run-autonomous` Step 0.7
...
3. **Shutdown / resume** — ask `run-autonomous` Step 2 **Q3** (shutdown yes/no)
   ...
   **Budget-Modus (`$BURN_MODE`, default Nein)** — one more `AskUserQuestion`:
   whether to work the backlog like `/run-burn` (budget-driven, aggressively
...
4. **Confirmation + timers** — arm the external watchdog (`register`) and the
   auto-resume cron (if shutdown=no + resume=yes) per `run-autonomous` Step 4
   and `skills/run-autonomous/deep-knowledge/shutdown-watchdog.md`. **Watchdog
   ...
   **backlog-runner-specific** marker — never the verbatim `AUTONOMOUS_AUTOSTART:` prompt,
   which `run-autonomous` Step 0.1 would catch and resume as its own no-ship
   ...
   CronCreate({ recurring: false, cron: "<now+3min>",
     ...
     /run-backlog Step 4 loop with: queue=<issue numbers>,
...
5. **Post-Confirmation Lockout** — after confirmation, ZERO blocking interaction
   ...
   `run-autonomous`. The only later interaction point is the next session.

## Step 4 — Per-Issue Lifecycle Loop (Autonom — user AFK)

**First, arm the autonomous lockout** so every composed sub-skill — above all
`/ship`, invoked once per issue below — runs non-interactively and never
...
`/ship` reads this in its Pre-Step A and turns each interactive gate
...
gate, run the queue in `/run-burn` style: first do a **budget assessment**
(`/run-burn` Step 2 — `refresh-usage-headless.js`, weekly-remaining %), then
drive each issue's IMPLEMENT with run-burn's aggressive agent parallelization and
throughput guidance (`skills/run-burn/deep-knowledge/composite-prompt.md`),
...
for each issue in queue:
  ...
  2. IMPLEMENT → the same role-agent orchestration /run-agents runs
                 (agent-orchestration.md — Single-Agent Shortcut / waves;
                 Autonomous directive, no AskUserQuestion). May delegate one heavy
                 item to a /run-autonomous implement sub-run (never ships).
  ...
  4. SHIP      → /ship (MCP ship tools) — this skill's own authority
  5. CLOSE     → close the issue; when ALL issues of a milestone are done,
                 close the milestone
  ── special cases ──
  • oversized discovered only now → refine it + prepare a /concept page +
...
- **Composed ships never prompt** — Step 4 arms the autonomous lockout, so
  `/ship` parks/blocks at every gate that would otherwise raise a modal.
...
- **Milestones are done when all their issues are closed** — close the milestone
  automatically at that point (`skills/setup-issue/deep-knowledge/milestone-rules.md`).
