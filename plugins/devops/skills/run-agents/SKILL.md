...
description: >-
  ...
  mode → waves → inter-wave gates → synthesis. This is the EXPLICIT path for
  Complex-tier work (3+ domains, a feature end-to-end, high-risk change); the
  everyday delegation — 1 background agent for research/tests/redteam, 2–3
  parallel agents for independent domains — happens automatically under the
  always-on delegation policy (deep-knowledge/agent-proactivity.md) and never
  goes through this skill. Triggers on: "run agents", "use agents",
  "orchestrate", "parallel agents", "multi-agent", "delegate to agents",
  "agent workflow", or Claude offering it for Complex-tier work and the user
  saying yes. Do NOT trigger for: simple edits, quick fixes, explanations, or
  single-agent research (the policy handles those inline or with one
  background agent).
...
1. Global: `~/.claude/skills/run-agents/SKILL.md` + `reference.md`
2. Project: `{project}/.claude/skills/run-agents/SKILL.md` + `reference.md`
...
options:
  ...
  - label: "Interactive"
    description: "Agents actively involve you on design/concept decisions — AskUserQuestion for short trade-offs, /concept for richer comparisons. Expect ≥1 checkpoint per wave."
```
...
options:
  ...
  - label: "Interaktiv"
    description: "Agents binden dich aktiv bei Design-/Konzeptentscheidungen ein — AskUserQuestion für kurze Trade-offs, /concept für komplexere Vergleiche. Rechne mit ≥1 Checkpoint pro Wave."
```
...
- **Interactive** (`$EXEC_MODE`): Use interaction directive "Interactive" from the
  ...
  `/concept` *before* spawning the relevant wave, not only inside it.
...
4. Remind the user to run `/ship` manually when ready to merge
