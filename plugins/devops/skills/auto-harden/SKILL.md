...
1. Global: `~/.claude/skills/auto-harden/SKILL.md` + `reference.md`
2. Project: `{project}/.claude/skills/auto-harden/SKILL.md` + `reference.md`
   Fallback (pre-PR-2 name): where `auto-harden/` does not exist, read `~/.claude/skills/tune-harden/` / `{project}/.claude/skills/tune-harden/` instead — an extension written before the rename keeps working.
...
4. For "Defer to issue" answers → call
   ...
   existing issue; if none, suggest creating one via `/auto-issue`.
...
same math, same thresholds; this phase is its whole-scope instance, `/do-ship`
runs the diff-only instance via `/auto-polish --invoked-by=ship`.
...
`skills/auto-concept/deep-knowledge/templates.md` (design system, panel,
...
step (e.g. "Ask for a polish pass", "Open issue", "Add test setup").
...
- **Never commit automatically** — this skill modifies files; the user
  commits via `/do-ship` (or an inline commit per `deep-knowledge/commit-conventions.md`).
