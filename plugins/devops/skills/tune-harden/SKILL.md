...
description: >-
  ...
  /tune-polish. Triggers on: "harden", "stabilize", "härten",
...
1. **Direct** — user types `/tune-harden` or trigger phrase. Full skill
   runs as documented below. Asks questions when `$AUTONOMOUS=0`.
2. **From `/run-agents`** — orchestrator detected a harden-style request.
...
3. **From `/run-autonomous`** — autonomous skill primed permissions,
...
1. Global: `~/.claude/skills/tune-harden/SKILL.md` + `reference.md`
2. Project: `{project}/.claude/skills/tune-harden/SKILL.md` + `reference.md`
...
4. For "Defer to issue" answers → call
   ...
   existing issue; if none, suggest creating one via `/setup-issue`.
...
`skills/concept/deep-knowledge/templates.md` (design system, panel,
...
step (e.g. "Run /tune-polish for this", "Open issue", "Add test setup").
...
- **No new features.** Ever. Even tiny ones. If it isn't a fix, a test,
  ...
  → it doesn't belong in `/tune-harden`.
