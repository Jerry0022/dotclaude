<!-- do-run mode `autonomous` — the body of the former `run-autonomous` skill (v0.6.0), moved verbatim in PR 2 of the skill restructure (docs/superpowers/specs/2026-09-24-skill-restructure-design.md). Its triggers, allowed tools and argument hint now live in ../SKILL.md. -->
...
2. `{project}/.claude/skills/do-run/SKILL.md` + `reference.md` — same
   Pre-PR-2 fallback: also read `~/.claude/skills/run-autonomous/` and `{project}/.claude/skills/run-autonomous/` (`SKILL.md` + `reference.md`) when present — an extension written for the old `run-autonomous` skill applies to this mode unchanged.
...
`{PLUGIN_ROOT}/skills/do-run/modes/autonomous/deep-knowledge/shutdown-watchdog.md` § External Watchdog. Run the `register`
...
CronCreate({
  ...
  recurring: false,
  prompt: "AUTONOMOUS_RESUME: token-window reset reached. Execute do-run autonomous mode Step 0.2 — resume hard-capped Claude worktrees with »weiter« (skip finished ones)."
})
```
...
Step 8 interaction: `{PLUGIN_ROOT}/skills/do-run/modes/autonomous/deep-knowledge/shutdown-watchdog.md` § Fail-Safe Shutdown Timer.
...
See `{PLUGIN_ROOT}/skills/do-run/modes/autonomous/deep-knowledge/html-report.md` for the full structure (header,
...
flag decision matrix) live in `{PLUGIN_ROOT}/skills/do-run/modes/autonomous/deep-knowledge/shutdown-watchdog.md` § Step 8.
