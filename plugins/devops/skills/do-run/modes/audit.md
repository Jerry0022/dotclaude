<!-- do-run mode `audit` — the body of the former `tune-audit` skill (v0.1.0), moved verbatim in PR 2 of the skill restructure (docs/superpowers/specs/2026-09-24-skill-restructure-design.md). Its triggers, allowed tools and argument hint now live in ../SKILL.md. -->
...
**Why this skill exists:** `/auto-harden` and `/auto-polish` improve code
...
2. Project: `{project}/.claude/skills/do-run/SKILL.md` + `reference.md` — same
   Pre-PR-2 fallback: also read `~/.claude/skills/tune-audit/` and `{project}/.claude/skills/tune-audit/` (`SKILL.md` + `reference.md`) when present — an extension written for the old `tune-audit` skill applies to this mode unchanged.
...
- `--dimensions=functional,visual,…` → restrict to these dimension IDs from
  `{PLUGIN_ROOT}/skills/do-run/modes/audit/deep-knowledge/dimensions.md`. Default: **all**.
...
2. For every dimension in `{PLUGIN_ROOT}/skills/do-run/modes/audit/deep-knowledge/dimensions.md`, run its
...
dimensions, the dimension checklists copied from `{PLUGIN_ROOT}/skills/do-run/modes/audit/deep-knowledge/dimensions.md`,
...
AUD-<nnn>  [<dimension>]  <severity: critical|high|medium|low|info>
  ...
  Route:     auto-fix | ask | auto-polish | auto-harden | issue | manual
```

Severity rubric: `{PLUGIN_ROOT}/skills/do-run/modes/audit/deep-knowledge/dimensions.md` § Severity. Requirement
...
- Structural UI changes (new controls, moved elements, layout change) are
  always `Route: ask` in interactive runs and `auto-polish` in autonomous ones.
...
Never commit — the user lands the result via `/do-ship`.

### 7b. `concept`

Nothing is changed. Invoke the plugin's `auto-concept` skill
(`Skill("devops:auto-concept")` — never a hand-built HTML page) with the audit dossier
(`requirements.md` + `findings.json`) and let its per-iteration template rule
(`skills/auto-concept/SKILL.md` § Step 1a) choose the layout — normally a
...
issues via `/auto-issue`, ship). Visual findings that need a before/after
...
VERBATIM as the last thing in the response. In concept mode the auto-concept skill
...
- **Implementation respects the same risk gates as `/auto-harden`** — score
