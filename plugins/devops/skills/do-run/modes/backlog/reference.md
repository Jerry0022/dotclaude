...
```
{project}/.claude/skills/do-run/
├── SKILL.md        ← Override or add steps (name the mode: "## backlog mode")
└── reference.md    ← Project-specific context
```

An extension from before the skill restructure at
`{project}/.claude/skills/run-backlog/` keeps working — backlog mode reads it
as well (`modes/backlog.md` Step 0).
...
- **Refine template**: extra sections to write back into each issue (test plan,
  ...
  `/auto-issue` refine mode, which places them in its managed section.
- **Ship gate**: project-specific pre-ship checks — these compose with the
  project's own `ship/` extension, which `/do-ship` already reads.
...
- This skill is the **ship instance** — it does not modify or route ships
  through autonomous mode. Extensions must keep the ship safeguards intact:
