...
description: >-
  ...
  Do NOT trigger for: PR creation (use /do-ship), plain commits,
...
2. Project: `{project}/.claude/skills/auto-issue/SKILL.md` + `reference.md`
   Fallback (pre-PR-2 name): where `auto-issue/` does not exist, read `~/.claude/skills/setup-issue/` / `{project}/.claude/skills/setup-issue/` instead — an extension written before the rename keeps working.
...
`# via auto-issue` marker comment shown above — `pre.issue.guard` lets a
...
(`/do-run backlog` Step 2 "Refine → issue", `/auto-concept` follow-ups, a user asking
...
```bash
gh issue edit {issue} --body-file "{tmp}/body.md"  # via auto-issue — add --repo "{target_repo}" when set
```
