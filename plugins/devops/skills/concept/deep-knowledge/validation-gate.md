...
| 33 | `renderCloseout` AND `buildFollowUpList` AND `buildCloseoutPlan` AND `initCloseoutRows` AND `closeoutButtonClick` | The sheet's renderers: block visibility + the done state, one row with three routes per open point, the live plan of consequences, the accordion's answered/open bookkeeping, and the one button's advance-or-execute click. Without the plan the single irreversible click is made blind; without the routes the user can only file or drop a follow-up, never ask for it to be built; without the accordion wiring every row shows at once with no way to tell answered from unanswered. |
...
| 36 | `status-channel` AND `updateStatusChannelSummary` (the CALL, inside `wireCloseout`) | Persistent status channel on the final-report panel — the always-visible pipeline recap (Übermittelt → verarbeitet → implementiert → Bereit) that hands over to the close-out sheet. Folded to one line by default (a `<details class="status-detail">`, same pattern as the submit-progress instance) with the full recap opening on click — without the call the summary line never fills in and the fold reads as empty. DOM-driven so it survives reload + stale heartbeat. See templates.md § Final Report Panel. |
| 37 | `closeout-sheet` — AND all four of `data-closeout-block="followups"`, `"ship"`, `"files"`, `"plan"` — AND `data-closeout-row` on each of the first three | The close-out sheet container, carrying the `data-plan-*`/`data-label-*` localised strings the plan and the accordion render from, plus its four blocks — the first three (followups/ship/files) collapsed to a `data-closeout-row` accordion head each, `plan` never gets one (it is read, not answered). Its absence means the final report has no way to close out at all; a missing block means a decision the user cannot express. A page still carrying `wizard-next` / `wizard-back` is the old step chain and fails this entry. |
