...
| 62 | `panel-toggle` AND `panel-backdrop` AND `panel-close` in the markup, `window.openPanel` in the JS — on EVERY page, whatever its templates — AND NOT `.panel-fab` inside any `html:not([data-template="design"])` rule | The ☰ decision panel is page chrome. It used to be design-only, with a docked sidebar standing in for it in decision/free rounds, so a concept that mixed templates moved its panel — and the surface the user writes feedback on — from behind the FAB into the page, mid-session. The panel wiring must also live OUTSIDE the design layout IIFE, which a document-only page never runs. See templates.md § Panel Chrome (all templates). ENGINE entry — see § Engine drift on iteration append. |
