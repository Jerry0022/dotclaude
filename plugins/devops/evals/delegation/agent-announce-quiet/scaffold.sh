#!/usr/bin/env bash
# Quiet output style active, as on a real install: the case pins that the
# pre.agent.announce line survives Quiet's "never narrate" rule.
set -e
git init -q . 2>/dev/null || true
git checkout -q -b eval/work 2>/dev/null || git switch -q -c eval/work
mkdir -p .claude/output-styles
cat > .claude/output-styles/quiet.md <<'STYLE'
---
name: Quiet
