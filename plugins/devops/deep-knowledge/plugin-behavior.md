...
- **A turn that ends while a background subagent is still running must render its
  card with `pending`** — naming each open agent, workflow or task (a foreground
  command the harness moved to the background at its timeout is one too). That replaces the CTA of
