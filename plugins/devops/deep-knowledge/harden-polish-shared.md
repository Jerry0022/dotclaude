...
Cross-cutting reference for `/auto-harden` and `/auto-polish`. Covers
...
These never appear in `/auto-polish` even with user approval — they
...
For Step 6 of `/auto-harden` (writing tests for untested critical
paths), prioritize by:
...
| `polish-candidates` | Items flagged for `/auto-polish` (harden only) |
| `harden-candidates` | Items flagged for `/auto-harden` (polish only) |
