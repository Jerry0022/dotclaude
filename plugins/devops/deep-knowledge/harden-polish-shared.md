...
Cross-cutting reference for `/tune-harden` and `/tune-polish`. Covers
...
These never appear in `/tune-polish` even with user approval — they
...
For Step 6 of `/tune-harden` (writing tests for untested critical
paths), prioritize by:
...
| `polish-candidates` | Items flagged for `/tune-polish` (harden only) |
| `harden-candidates` | Items flagged for `/tune-harden` (polish only) |
