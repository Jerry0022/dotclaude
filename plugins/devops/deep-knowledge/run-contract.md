...
A contract exists only after do-run's router answered, or a machine prompt armed one. Work that meets do-run's own criteria but that never went through do-run (or a machine-prompt run) is not gated at all — outside an active run the delegation policy stays advisory (a kill switch, not a gate) and the trigger router only suggests a skill. The one place outside a run that still forces a skill is the do-batch hand-off gate below; the 6th distinct changed file of a turn without a run gets one non-blocking mention of `auto-agents` (`agent-proactivity.md`), never a gate. The command reading has known gaps (a Windows path ending in `\` right before its closing quote; cmd's `start` and commands built at runtime are out of scope; `git commit-tree` is no commit — see the spec's Limits), and two sessions in one checkout share one contract file (the later router answers replace the earlier run's contract) — use a worktree per session.
