 ...
 * Background
 * ----------
 * Many devops skills (concept, do-run autonomous mode, …) write a local HTML
 * artefact inside the current worktree and open it via `start msedge "file://…"`.
 * When `/do-ship` later runs `ship_cleanup`, the worktree directory is
 ...
 * main-repo equivalent path. Used by `/do-ship` Step 5 after
...
function cmdReopenMain(argv) {
  ...
  // Single-line summary so /do-ship can relay it.
  const summary = {
    mainRoot,
    worktree: wt,
    reopened,
