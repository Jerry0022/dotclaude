...
describe("conflict reporting", () => {
  test("auto-resolves a whitespace-only conflict instead of asking for help", async () => {
    ...
    await advanceOrigin(other, "base.txt", "BASE\n", "main rewrites the line");

    write(wt, "base.txt", "  base\n");
    await commitAll(wt, "worktree only re-indents the line");
  ...
  test("rolls the merge back when the auto-resolved commit cannot be made", async () => {
    const { root, wt, other } = await makeWorld();
    await advanceOrigin(other, "base.txt", "BASE\n", "main rewrites the line");
  ...
  test("an ambiguous conflict is reported with its file list and aborted cleanly", async () => {
    const { root, wt, other } = await makeWorld();
    await advanceOrigin(other, "base.txt", "written by main\n", "main rewrites base.txt");

    write(wt, "base.txt", "written by the feature branch\n");
    await commitAll(wt, "feature rewrites base.txt");
  ...
  test("an ambiguous conflict survives unrelated dirt in the worktree", async () => {
    const { root, wt, other } = await makeWorld();
    await advanceOrigin(other, "base.txt", "written by main\n", "main rewrites base.txt");
  ...
  test("stays silent when the merge would overwrite an untracked file", async () => {
    const { root, wt, other } = await makeWorld();
    await advanceOrigin(other, "generated.txt", "from main\n", "main adds generated.txt");
  ...
  test("detects an overlap when main renamed a file the worktree is editing", async () => {
    const { root, wt, other } = await makeWorld();
    await git(other, ["mv", "base.txt", "renamed.txt"]);
    await commitAll(other, "main renames base.txt");
...
describe("branch hierarchy", () => {
  test("merges the whole parent chain from remote-tracking refs", async () => {
    const { root, primary, other, originPath } = await makeWorld();
    ...
    await git(other, ["checkout", "--quiet", "feat"]);
    write(other, "from-feat.txt", "feat\n");
    await commitAll(other, "feat moves");
    await git(other, ["push", "--quiet", "origin", "feat"]);
    await git(other, ["checkout", "--quiet", "main"]);
    void originPath;

    const report = await runSync(sub, path.join(root, "result"));
  ...
  test("ignores a parent segment that does not exist upstream", async () => {
    ...
    await git(primary, ["worktree", "add", "--quiet", "-b", "claude/x", sub, "main"]);
    await advanceOrigin(other, "from-main.txt", "main\n", "main moves");
