...
- Concept pages: every hover hint is `data-tip` through templates-utilities.md § App Tooltips, every scrollbar is the universal skin in templates-design-css.md § Layout CSS → Scrollbars, all colours from the page tokens (`--panel-bg`, `--text-color`, `--border-color`, `--accent-color`).
