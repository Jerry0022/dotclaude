...
### Changed
- **The graphify gate answers instead of nagging, and it now actually applies** — an audit of `~/.claude/graphify-metrics.jsonl` found 0 `graphify query` runs in the last 10 sessions. The gate only fired on path-less Grep/Glob, and the project-map injection teaches Claude to always pass `path`, so 652 of 691 searches were never gated (last `gate_fired` 2026-09-20); one query also relented the gate for the whole session. `pre.tokens.guard` 0.13.0 now gates eligible **Grep** only (path-less, or `content` mode on a directory inside the graph root, no `glob`/`type` filter, 1–4 identifier-like terms with at least one specific term, via `graph-nudge.isEligibleSearch`). The hook runs `graphify query "<terms>" --budget 400` itself and blocks with the answer (file:line nodes) in the message, so no second call is needed. No hit, timeout (4 s), error, or both machine-wide query slots busy → the search runs, and a "declined" marker keeps a repeat from ever being gated. Retrying the same search is the escape hatch and also releases the classic full-repo block (no double block); 3 bypassed answers on different searches relent the gate for the session. Gate state has a 12 h TTL with a SessionStart sweep; a missing session id skips the gate. Glob is no longer gated by graphify. Measured idle: hot path +0–4 ms, a gated query 1.1–2.0 s, answers ~1.65 k chars, retries at hot-path cost.
- **Graphify telemetry measures cost and savings at zero context tokens** — every event still goes only to the jsonl (`DOTCLAUDE_GRAPHIFY_METRICS` overrides the path; all hook-spawning tests use it). New fields: `query_ran.responseChars`/`budget`, `search_ran.outputMode`/`pathKind`/`eligible`, `gate_fired.answerChars`/`keyHash`/`ms`, `gate_noanswer.reason`/`ms`, `gate_skipped_busy`, `gate_relented`, `guard_blocked`/`guard_released`. `scripts/graphify-audit.js --sessions N` (default 10) prints a per-session table, gate latency as a cost line, and an unclamped NET savings estimate (bypassed gates count as losses, baseline split by output mode, legacy events reported separately). Its transcript mode puts search results next to total new input: 0.35 % over the last 10 sessions, the ceiling for any search gate.

### Fixed
- **The token guard no longer blocks a Bash command that only mentions a big file** — `ls -la graphify-out/graph.json; …; node -e '…' x` was blocked as "~1,097,158 tokens": once any segment read something, `matchCostlyFiles` matched the whole command. The widening is now scoped to its statement/pipeline (`bash-context-cost.groupedSegments`), and every Bash estimate is capped at what the harness can return (`BASH_MAX_OUTPUT_LENGTH`, default 30000 chars → 7.5 k tokens). At that default the Bash large-file branch cannot block at all; it only matters when a user raises the limit.
- **The graphify query spawn cannot run shell commands** — the first version spawned with `shell: true` on Windows. Node joins args unescaped there, so graphify received only the word "What" and a Glob pattern like `**/*&calc&` could execute programs. `hooks/lib/graphify-query-spawn.js` spawns with `shell: false`; only an explicit `.cmd`/`.bat` override falls back to a strictly quoted shell, and that fallback refuses any argument containing `%"!^` or a line break. The question is also sanitized to `[A-Za-z0-9 ]`. Found by red-team review and confirmed live by QA before merge.
...
- **Completion card on Desktop: drawn once, whole, visible.** Observed after 0.182.0: the body widget and the full markdown card rendered one under the other — the whole card twice; result lines were cut at 120 characters with an ellipsis, which read as "the card only shows half"; and the panel's `--surface-1` was indistinguishable from the page in dark mode ("backgrounds are not rendered"). Now the widget carries the **title** (h2) and both blocks, and the markdown under it shrinks to the `### **✨✨✨ {title} ✨✨✨**` line — the card-guard marker and the transcript record, which also keeps the turn ending on visible text (`renderCard(…, { titleOnly })` when `CLAUDE_CODE_ENTRYPOINT=claude-desktop` and the variant has a body; `test-minimal`, which never calls the widget, keeps its markdown). The widget wraps result lines instead of clamping them (`buildResultLines(…, { clamp: false })`; the 120-char ellipsis stays a terminal budget), the panel sits on `--surface-2` with a `--border` hairline, and `cardSignature` (§ 5.5 duplicate detection) falls back to the title for a title-only card. The in-chat buttons were re-verified live: the "Trotzdem shippen ↗" click of this very ship landed its prompt in the session. Design doc § 4, the template and the CLI/widget/guard tests pin the new split; the card tests pin `CLAUDE_CODE_ENTRYPOINT=cli` so a vitest run started from a Desktop session asserts the terminal markdown.
- **Session title: a ship prompt marks `🚀 Shipping – ` itself; the hourglass is the fallback, never the override.** Observed: `/ship` after a change left `⏳` on the sidebar for the whole pipeline whenever the model skipped the ship skill's courtesy rename (Pre-Step C), and sometimes flipped back and forth. `prompt.flow.title-work` 0.4.0 classifies the prompt with the new `hooks/lib/ship-intent.js` — the one keyword list, shared with `prompt.ship.detect` 0.3.0 — and emits `🚀 Shipping – ` for a ship prompt; the hourglass instruction never replaces a running `🚀 Shipping – ` (only the outcome `🚀 Shipped – ` is stripped). A question (`fertig?`, `ship it?`) is a status request, not a ship order, in both hooks. Ship SKILL Pre-Step C finds the title already marked and leaves it; design doc § 7 and `plugin-behavior.md` state the precedence rule (process prefixes 🚀 Shipping / 🧭 Concept / 📥 Batch outrank the bare ⏳).
- **`ready-red` heading names what is red.** An unmet requirement routed to `⚠ Trotzdem shippen mit 1 roten Tests?` over a green suite; the heading now says `… mit N roten Tests` only for failing tests, else `… mit N unerfüllten Anforderungen`, else `… teilweise erfüllten …` (de + en), and the Fix / Trotzdem-shippen button prompts speak of "rote Befunde" instead of red tests. Not covered by Codex review — external usage limit until 2026-10-11; full suite 145 files / 3591 green (7 load timeouts in four unrelated suites, all green re-run in isolation), lint clean.
...
- **Completion card: one page, three result lines, one decision.** The old card had to be scrolled to be read — a `Changes` block with file names, a `Geprüft` block with five checkmarks, `⚠ OFFEN` in the middle, the usage meter between body and footer, a 📌 build line, a state line, a CTA heading, and the clickable verbs *above* it all. Redesigned on an 11-round `/concept` page with the user (mock-ups of every state, 16 scenarios on the final layout, 12 adaptations accepted) and specified in `deep-knowledge/completion-card-design.md`, which the renderer, the widget and the guards now implement. The card is two blocks: **what happened** — the `### ✨✨✨` title (outcome only, never status), up to three `›` result lines that answer the prompt (a deviation is line 1, `**Nicht erreicht:**`), an evidence row of always the same three posts (`✓ 3/3 Anforderungen  ✓ 3464 Tests grün  ✓ 4 Live-Checks ok` — deviations first and bright, lint/build/review/the V&V stamp only with a finding, monochrome glyphs `✓ ✗ ◐ ○ ●`), a budget line (bar = elapsed time, marker = usage, remaining time as a watermark, omitted while both windows are < 50 % and > 1 h from reset; `🧠 1180 Calls · /compact` dim at its end past the threshold) and a pipeline line (`○ commit → ○ push → ○ PR → ○ merge · branch · Build 176c57d`, continuing `→ ✓ alpha → ○ beta → ○ stable` on ring projects — it replaces the Delivery ladder); **what to decide** — an H2 heading that *is* the decision (`📦 Shippen trotz fremdem Testfehler?`, `⚠ Trotzdem shippen mit 2 roten Tests?`, `⛔ … umgehen und trotzdem shippen?`, `🚀 Released v0.182.0 alpha — nach beta promoten?`, `🎊 Promoted v… BETA — nach stable?`, `🎊 Released v… LIVE — stable.` as a state when nothing is left to decide), an optional `›` context line, at most three numbered points (reservations, then manual test steps, then deploy artifacts; more → `+N weitere`), and — on Desktop — the buttons. Every variant and override (ready, red, blocked, shipped, kept, deploy-pending, released beta/stable, ready-files, test, test-minimal, analysis, aborted, fallback, pending, concept, batch, unverified) maps to one row of the design's § 3 table, de + en; the terminal gets the same card without buttons. Heuristics the concept settled: tests read as number + noun + state (`3464 Tests grün`, skipped is never a deviation, `0 rot` is green), live checks are counted, gates without a lane (preflight, smoke) surface only with a finding and by name, a short reservation is quoted in the heading and a long one becomes the count, an identifier-like area keeps its subject (`run-agents nutzt …`) while a worded one is dropped (#396), the ring pipeline ticks only the channels *this* version reached.
- **Desktop: the whole card body is one widget** (`mcp-server/lib/card-widget.js`, replaces `cta-actions.js`): the soft "what happened" panel and the accent-bordered decision box, green/red/yellow evidence posts with tooltips, budget bars with time fill, usage marker (white ≤ 10 pp ahead of time, yellow ≤ 25, red beyond), watermark and a light band inside the fill, a quiet PR link, code spans in the accent lilac instead of the client's red, equal-height buttons with tooltips. `[CARD WIDGET — DO NOT OUTPUT THIS BLOCK]` replaces `[CTA ACTIONS]` and still renders before the card markdown; `test-minimal` never calls it.
- **Session title: ⏳ (icon only) while working instead of 🔧** (`prompt.flow.title-work` 0.3.0); the card's result prefix replaces it at turn end. `mode-state.js`: `work: "⏳ "`, the fallback card gets its own `🔧 Done – `. The strip lists in the hook and the card cover every state (Shipping/Shipped, Released Alpha/Beta/Stable, Ready, Blocked, Test, Started, Analysis, Aborted, Working, Concept, Batch).
- **Guards for the new anatomy** (`card-guard.js` 0.5.0, `stop.flow.guard.js` 0.5.0): a title with status words (`läuft`, `wartet`, `pending`, agent counts) is blocked once with the reason; more than three `›` result lines, or one whose first token is a path / hook name, is blocked; more than three points is blocked; the line budget (14 Desktop / 24 terminal rows) is reported; a turn that starts from a background-task notification, wake-up or cron tick carries **no card obligation** when nothing changed, and an identical second card (same variant, build-id, evidence row) is blocked — the "Hintergrund-Testlauf beendet — Stand bleibt" echo after a card is gone. `templates/output-style-quiet.md` states the same rule for the style. `prompt.flow.appstart` names the new `test-minimal` / `test` headings; `post.flow.completion` looks for `[CARD WIDGET`.
- Docs: README § Completion Cards rewritten to the new anatomy (one full example, a compact variant table instead of thirteen old-format cards), `plugin-behavior.md`, `templates/completion-card.md` as a pointer to the spec, deep-knowledge index and architecture roster regenerated, `ship/SKILL.md` prefix reference. Follow-ups filed from the concept's close-out: #423 (build-id.js fails on a deleted-but-tracked file), #424 (git-sync tests time out under full-suite load). Not covered by Codex review — external usage limit until 2026-10-11; full suite 144 files / 3554 green before the last renderer fixes, `ship_build` ran the 37 card / guard / title suites (825) under its 120-s ceiling after them, lint clean, six variants rendered through `--render-card` and read.

## [0.181.0] — 2026-09-20

### Fixed

- **`/concept`: "the connection keeps dropping" — the bridge was never dead, its watchers and the page read a live bridge as dead.** Measured on the five bridges running that day plus the transcripts: (1) 158 `PULSER_EXIT` / 171 `WAKER_EXIT reason=SERVER_DEAD` — an 8-s request deadline × 4 misses on a box where `/heartbeat` took 10–20 s under load (four agents and a build; a plain `wc -c` took two minutes), after which the waker POSTed `/shutdown` or Claude relaunched (journals show shutdown/restore pairs minutes apart, page red in between). (2) `STATE_GONE` / `HTML_GONE` on a *single* poll while Claude rewrote the state file or the page — tmp+rename / `mv` leave an `existsSync` gap on Windows. (3) Two wakers waking two Claudes for one submission (`PENDING_SUBMISSION` lines 4 s apart): `ss.concept.resume` re-arms the watchers on every session start and the old detached tasks survive it on Windows — three pulsers per port after two restarts. (4) The page judged a stale `claude_ts` from a sample that predated a tab sleep or PC suspend → "Nur lokal gespeichert · getrennt" flashed on every return to the tab. `concept-watch.js` 0.3.0: requests wait 30 s; a cleanup verdict must hold for `--confirm` (3) polls; the pulser never exits on request failures while the state file exists (it reconnects to a relaunched bridge); the waker exits `SERVER_DEAD` only after `--dead-after` (300 s) of continuous failure; every watcher carries an id and the server echoes the previous poller (`POST /heartbeat?pulser=` → `prev_pulser`, `GET /pending?waker=` → `prev_waker`) — the younger pulser and the OLDER waker step down (`DUPLICATE_PULSER` / `DUPLICATE_WAKER`, both in the reason→action table). `concept-tick.js` 30-s timeout. templates.md § Claude Connection Heartbeat: freeze-aware verdict — sample age before `claude_ts`, immediate re-poll + worker replacement after a freeze, 45-s wake grace, two-strike "disconnected", single-flight poll, `visibilitychange` re-poll (gate 3d). Live-verified on a scratch bridge (dedup, 3-s rewrite windows, a 12-s outage) and by injection into an open page. 69 watcher tests, 5 freeze tests, server echo test.
- **`/concept`: a ☰ page row clicked while a view was on screen did nothing.** The hidden design still carried `data-design-active`, so the per-screen handler took the same-design branch and `showScreen()` swapped pages inside the hidden design; only the top-centre switcher recovered. The handler routes through `showDesign(d, sec.id)` whenever `body[data-view-active]` is set. Regression pinned in `design-nav-guard.test.js`.

### Added

- **`post.concept.gate` blocks a page whose engine was lifted from an older concept page (STALE ENGINE).** A fresh page on plugin 0.180 opened with the September-14 panel — no rounds chip, no viewport toggle, no freeze-aware heartbeat — because the generating session had read an older page of the same project for reference and copied its `<style>`/`<script>`/panel wholesale instead of templates.md; every REQUIRED marker was present, so the gate let it through. `concept-gate.js` gains `ENGINE`, six engine-currency anchors (`panel-here`, `panel-status`, `renderPanelStatus`, `buildRoundsChip`, `buildIterationTree`, `recoverFromFreeze`); a page missing any is blocked with the re-sync instruction, and a test pins every anchor to templates.md so the list cannot drift from the reference. SKILL.md Step 2 § Engine source states the rule: the engine comes verbatim from templates.md of the running plugin, older pages in `docs/concepts/` are content references only. Existing pages get the new heartbeat block forced on their next append (Step 2.6 engine drift).
- **`/concept`: the round label and the 🕘 rounds chip moved into the ☰ panel's head row.** "Iteration 6" sat on its own line under the ☀️/✕ row — a line of panel height spent on nothing. Both skeletons now read `Iteration 6 · 🕘 5 · ☀️ · ✕` on one line (frozen round: `Iteration 1 · archiviert · ↩ zur Runde 6 · ☀️ · ✕`); `#panel-here` keeps the "› TOC entry" sub-line and the unfolded rounds list. Preview confirmed by the user before the change landed. Not covered by Codex review — external usage limit until 2026-10-11; `ship_build` ran the nine concept/watcher/gate suites (299) under its 120-s ceiling, the full suite ran separately.
...
- **A window reset is announced, not left to silence — and the background refresh is plan-scaled and never fires at the limit.** Incident 2026-09-20: a session that had hit the weekly limit was retried after the reset with a 16-char prompt ("Erneut versuchen"). The hook had the right data (window expired → `free`, scraper started) but emitted nothing — no nudge (short prompt), no suffix (`free` is silent), no SessionStart (the process never restarted) — and the model wrote "Wochenbudget ~100 %" from the transcript into its own `/run-agents` args, which then ran the 7-agent plan on `sonnet-only` / ≤ 5 calls until the user objected. PO + redteam review (parallel): an emission problem, not a freshness problem, and usage is monotonic until a reset, so an ageing snapshot only ever drifts *optimistic* — the two ways a class is wrongly tight are a missed reset and transcript memory. Fix, stateless on purpose (a per-session "last emitted" marker would add compaction / parallel-session / tmp-cleanup failure modes): (1) **positive signal** — while a window is past its reset or reset recently (5 h: first hour, week: first day) the full `[budget]` line goes out on every prompt regardless of length, names the reset (`week reset 2 h ago … → free`) and states that earlier limit messages and usage claims no longer apply; SessionStart `resume` emits the line too. (2) **Refresh due by headroom, not a flat age** — the detached scrape starts when the worst-case burn since the reading could have crossed the next class threshold: headroom × window fill time per plan (Pro 30 min, Max 5x 150, Max 20x 600; the week ≈ 8 windows) — Max 20x at 85 % after 30 min, Pro at 10 % after 18 min, Max 20x at 40 % only at the 3-h stale mark; at `sonnet-only` nothing above can change, so only the reset triggers it, and a reset ≤ 10 min away skips a now-worthless reading. A failed scrape is reported (`refresh failed: not logged in — run /auto-usage`) instead of "refreshing" and backed off for 30 min. (3) **`get_usage` returns `budget`** (tier / cls / binding / line) from the same `lib/budget.js` as the hooks, classified from the in-memory result so class and `cached`/`failureReason` always agree; the error path still carries a class. (4) **Policy**: the newest `[budget]` line is the class — it outranks earlier *usage claims* (limit messages, own plan text, skill args) but never the user; `/run-agents` Step 3 falls back to `get_usage` only when no line exists. Only `run-agents` needed a gate — `run-burn` refreshes already, `run-autonomous` / `run-backlog` make no class decision. Always-on preload cap 6 → 7 KB for the grown policy doc. budget 0.3.0, dispatch hook 0.7.0, index hook 0.6.0; 21 new tests. Not covered by Codex review — external usage limit until 2026-10-11; full suite 3461 green with seven `git-sync.*` 60-s timeouts under machine load (unrelated, pass in isolation).
...
- **Clickable CTA verbs on the completion card — Desktop app (#389).** No client turns a markdown link into "send this prompt into the CURRENT session": `claude://` and `claude-cli://` deep links open a NEW session and never auto-send, the terminal hands a custom scheme to the OS, `send_message` refuses the current session. The one mechanism that reaches the running session is the Desktop app's inline widget (`mcp__visualize__show_widget`) and its `sendPrompt()`. So the card keeps its markdown CTA everywhere, and in a Desktop session the tool result (stderr on `--render-card`) carries a `[CTA ACTIONS — DO NOT OUTPUT THIS BLOCK]` block with a ready-made one-row widget Claude renders right **before** the card: one control per CTA verb — `ready` Ship / Ändern, `test` Ship / Nachbessern, `ship-blocked` Fix / Skip, `ship-successful` on a ring project Promote, deploy-pending Deploy, `released → beta` Nach stable, `analysis` Umsetzen / Frage, `aborted` Nochmal — primary verb accented, a click sends the matching prompt (slash command or a self-sufficient sentence, never a half sentence) into this session. Nothing on pending / concept / batch overrides, test-minimal, fallback, ready-files, a kept ship or a finished stable release. Two live findings pinned in code: the controls are `<span role="button">` because from a real `<button>` the host's `sendPrompt()` runs but nothing reaches the chat, and the widget goes before the card because a turn ending on the tool call has no visible text and gets bounced. New `mcp-server/lib/cta-actions.js` (dependency-free, gated on `CLAUDE_CODE_ENTRYPOINT=claude-desktop`), wired on both card paths; stop-guard and post-tool-use texts, the card template (§ CTA Actions) and README follow. Completion server 0.6.0; 17 unit tests + a CLI test pinning Desktop == terminal markdown.
- **`/setup-issue` refines existing issues and is the single owner of every issue write.** New **refine mode** (an issue number / URL, or a caller hand-over `{ issue, refinement }`): R1 fetch + diff, R2 one managed `## Refinement` section between `<!-- refinement:start/end -->` markers (acceptance criteria, implementation plan, resolved decisions, `**User value:**` only when the body has none) that a second run replaces instead of stacking, R3 title / `type:*` / milestone corrections + verify. A complete **caller hand-over** makes the skill non-interactive, so zero-prompt flows delegate too: `/run-backlog` Step 2.3 hands each refined issue over instead of `gh issue edit`, `/concept` `create-issues` hands the gated items over instead of `gh issue create`; invoked mid-flow the skill returns without a card. `plugin-behavior.md` "Issue Creation & Editing — Always Delegate" now covers `gh issue edit`. The four open issues (#406, #400, #399, #389) were refined through the new path and the repo gained the `type:*` label scheme the convention expects. Skill 0.3.0; run-backlog 0.6.1; concept 0.1.2. Not covered by Codex review — external usage limit until 2026-10-11.
...
- **The delegation policy knows the plan.** Delegation spends volume where opus·high agents are spawned — model × effort × tool calls — and on a Pro plan even one of them is a real share of the 5-hour window, while on Max 20x two in parallel are noise. `hooks/lib/budget.js` classifies every session from plan × 5h window × week, read locally (`rateLimitTier` from the credentials — only that key — first, the usage snapshot's label as fallback; percentages from `usage-live.json`) and injects one `[budget] Max 20x · window 56% (reset 51 min) · week 43% → free` line at SessionStart plus a suffix on the per-prompt nudge. The classes are named by what they do: **free** (tiers as designed), **ask-before-parallel** (Pro from 0 %, Max 5x ≥ 80/90 %, Max 20x ≥ 90/95 %, or an unknown plan with unknown usage — single agents run on `sonnet` with a ≤10-call ceiling; a parallel spawn or a ceremony asks ONE question per session, three options: spare the window / the right agents regardless / inline; the answer holds for the whole session and every tier, nothing is persisted, a new session asks again), **sonnet-only** (Pro ≥ 70/85 %, Max 5x ≥ 95/98 %, Max 20x ≥ 98/99 % — single agents on `sonnet` with ≤5 calls, the question names the binding reset). The Agent tool has no effort parameter, so the spare path uses the two levers it has: the model and a tool-call ceiling in the agent prompt. Explicit `/run-agents`, `/run-autonomous`, `/run-backlog`, `/run-burn` are the "full" answer — never asked, never downgraded (run-burn upgrades on purpose) — and the dispatch hook drops the suffix on `AUTONOMOUS_*` prompts and under the lockout sentinel. Override for tests and power users: `DOTCLAUDE_BUDGET` / `EVAL_DOTCLAUDE_BUDGET` with a class name. Eval `parallel-two-lenses-pro-budget` pins the Pro path (measured: one `devops:research` on `model: sonnet`, no second agent).

### Changed

- **The delegation policy after a red-team and a product-owner pass.** Sixteen red-team findings and twelve PO challenges, applied: the parallel tier is split by side effect — two *analysis* lenses spawn automatically, two *implementing* agents on the working tree are offered like a ceremony; hard stops apply only when the request is narrowed ("nur X", "keine Agents"), not to filler ("kannst du einfach mal prüfen…"); hard-go signals ("agents", "full") skip the question; every spawn is announced with its mode (`→ research agent (sonnet, ≤10 calls, spare): …`); the spawn template carries the budget class down to agents that spawn agents (`feature`, `designer`), since hooks never fire inside sub-agents; options are localized per `[ui-locale]`. The policy is re-injected on `source=compact` — compaction keeps the session id but drops the earlier injection — and tells the model to re-ask when it cannot know whether it asked. The per-prompt nudge (589 bytes) is skipped for prompts under 40 characters: an "ok, weiter" can never be non-Inline and every copy stays in the transcript for the rest of the session.

### Fixed

- **A usage snapshot whose window has reset contributes no usage.** Desktop sessions never run the status line, so `usage-live.json` is whatever the last terminal session wrote — an evening that ended at 96 % made every next-morning session `sonnet-only` all day, silently downgrading every research agent. A window whose `resetInMinutes` has elapsed since the snapshot's timestamp now reads as unknown (`usage unknown (snapshot past its reset)`), and the class falls back to the plan rule alone; the binding window (week vs. 5h) is named in the line and in the question.
- **The plan label no longer sticks.** The status line copies its last `plan` label forever, so an upgrade from Pro stayed "Pro" and a downgrade stayed "Max". The credentials tier is read first (cached once per session in a temp file — the credentials file is not parsed on every prompt) and the label is only a fallback.
- **`prompt.skill.enforce` no longer reads machine turns as the user.** A background agent's report arrives through UserPromptSubmit like any prompt; a red-team report that *discussed* `/run-agents`, `/run-autonomous`, `/run-backlog` and `/run-burn` demanded four Skill loads. Task notifications, system notifications and channel messages are skipped.
- **`plugin-guard` compares real paths and honours `settings.local.json`.** A junction, a symlinked cache or a HOME≠USERPROFILE mismatch could flip the ad-hoc-load rule either way; a plugin enabled only in `settings.local.json` was silenced.
- **Eval graders measure the delegation decision, not the card ceremony.** Every role-agent grader excludes spawns whose input mentions the completion card (without Bash the Stop hook's offline render is delegated to whichever agent has a shell — `devops:core` was observed doing it); "said X" graders are anchored on the assistant role, since the user turn itself carries the nudge text; each case pins its budget class through `EVAL_DOTCLAUDE_BUDGET` so the `[budget]` line is real hook output rather than a simulated contradiction.
...
- **The ship card fits one screen.** An analysis of the last 100 ship cards across five projects (6–13 Sep) measured what the card had become: median 62 visual lines on a normal desktop chat column, every card with at least five lines over 100 characters, the version repeated five times per card (title, Delivery, ladder, 📌 footer, CTA), the PR title twice, "merged → origin/main" three times, validation bullets at a median of 208 characters (max 782) taking 22 % of the card, the `⚠ Pace!` flag on 91 cards and the "consider /clear" note on 78. Title, CTA and footer — the parts meant to pop — were 7 % of the area. The renderer now enforces **character budgets on every body field** (`clampEllipsis`: word boundary + `…`): summary 60, change area/description 24/90, gate method/result 40/60, validation requirement/evidence 70/100, PR title 70, pending "doing" 90 (that one used to share the 48-character name cap and cut mid-word on 31 of the 100 cards). More than three changes render an explicit `* +N weitere` tail instead of vanishing. Tests and validation become **one `Belegt` / `Verified` block**: the automated gates on the header line (`npm test → 1460 grün · eslint → sauber`, bullets when the line would exceed 110 characters or more than three gates ran), validation bullets underneath with `⚠️ partial` / `❌ unmet` first and met overflow collapsed into `✅ N weitere Anforderungen erfüllt`. **Delivery is one block at the foot of the body** — PR · base + version bump · commit · build-id · horizontal channel ladder (`🟢 alpha \`v0.156.0\` · ⚪ beta · ✅ stable \`v0.153.0\``) — and replaces the 📌 footer and the "updated origin/main · merged PR …" state line; a project without channels shows no hollow "⚪ Promote" any more, and a commit SHA passed as ship version no longer renders as `vb43bf60`. The CTA says the destination once (`🚀 SHIPPED → alpha — Alles ERLEDIGT`, `→ main` without a ring) and drops the merge-target echo. The Pace flag fires from +20 pp instead of +10, the context-health note from 1000 / 2000 tool calls instead of 120 / 200, and it is kept when the usage scrape is unavailable. Variants without a delivery track (analysis, test, fallback …) keep the classic footer + state line. The template (`templates/completion-card.md`) documents the structure, the budgets and the new blocks; the README example shows the compact card.

### Added

- **`open` — a block for follow-ups that are not tests.** On the analysed cards, 95 of 198 items under `🔬 TESTE bitte noch` were decisions, cleanups or open questions, and 45 cards carried the promote nudge there — a header that asked to *test* something nobody can test. `open: [...]` renders its own `⚠ OFFEN:` / `⚠ OPEN:` block right after the test block (items never cut), with the same admission rule as the concept skill's open points (#367): only what the user deferred or what turned up outside the scope, never the approved scope's next step or a generic nudge. The promote nudge moves onto the Delivery ladder line via `delivery.promote.stableLag: { versions, days? }` (`· alpha 8 Versionen / 7 Tage vor stable → \`/promote\``); the ship skill's Step 6 passes it there instead of a `userFinalTest` item, and its payload template now spells out the summary rule (what changed for the user, ≤ 8 words, no version and no "gemergt/live"), the gates format and the validation budgets.
...
- **An open point in a final report can now be built instead of filed.** The close-out's open-question checkboxes could say "make a GitHub issue" or "forget it", so the thing users most often wanted — "just do this one now" — had no expression at all. Every row carries three routes now (Issue / Jetzt umsetzen / Ignorieren), the payload splits into two disjoint buckets, and `finalize` executes them in a fixed order: issues → implement → ship → cleanup. Implement lands before the release, or the release would predate the work it is supposed to contain; issues are created first because they are the cheapest and most independent part, so a run that dies mid-build still leaves the follow-ups tracked. The built items are noted in the report as `✓ umgesetzt`, their checkbox disabled, which is what drops the row from the sheet on the next render.
- **"Mit Feedback implementieren" runs through the devops role agents.** An implement order is the one place in a concept session where code gets written, it is usually multi-domain, and the main session has a second job while it runs (heartbeat, status POSTs, checkpoints). It is dispatched by domain now — `devops:core`, `devops:frontend`, `devops:designer`, `devops:ai`, `devops:windows`, with `devops:qa` verifying — the way `run-agents` does it. Inline execution stays available for a change small enough that one dispatch costs more than it saves, and must be named in the final report when taken.

### Changed

- **The close-out is one sheet, not a four-step wizard.** Reported from real use: the step chain was unreadable. "Weiter" between questions looked like it might already have committed something, "Alles ausführen" and the per-step Weiter were two different kinds of commitment on the same screen, and the plan of consequences — the one thing that licenses an irreversible click — only appeared on step 4 of 4. All three questions now sit on one sheet in execution order with the live plan directly above a single execute button; nothing commits before it. The ship question still has no default, and refusing to answer it explains the block instead of disabling the button. "Verwerfen (Standard)" — one word away from the bi-state *Verwerfen* on every variant card, and read as "throw the work away" when it only ever deleted an HTML file — is now "Seite löschen", with a hint saying the implementation stays.
- **The ☰ decision panel is page chrome: one overlay, every template.** It used to dock into a ~20% sidebar for `decision` and `free` rounds and hide behind the FAB only in `design` ones, so a concept that mixes templates moved its panel — and with it the surface the user writes feedback on — mid-session. Its markup, CSS and wiring live in one shared section now, which a document-only page carries without taking the design canvas with it. What stays per-round is only where feedback is written: the 💬 dock over a mockup, inline note fields in a document round, mixed freely. An open panel is modal in the bargain — nothing scrolls behind it, Escape closes it, and the 💬 FAB no longer paints over it.

### Fixed

- **A round could silently swap the reviewer's whole feedback surface.** Found on four real generated pages: a reality-check round appended as `decision` inside a three-round `design` concept hid the FABs and the dock and moved every note field into the cards, and a final report appended with no template at all resolved through the mutable `<html data-template>` projection — so the same report rendered as a fullscreen canvas when reached from a design tab and as a document when reached from a decision tab. The fallback is the concept's base template now (the lowest-numbered section that declares one, read once at load), the attribute is mandatory on every appended section, and the continuity rule — a design concept stays design; non-visual questions go into a `data-view-kind="decision"` view; the final report is always `free` — is written where the appending Claude reads it. Gate 12b hardened, 12c and 62 added, with 62's negative catching the dangerous mixed page: a leftover sticky sidebar rule wins on source order and leaves an unopenable panel behind a dark backdrop.
- **Two adversarial reviews of the above, and the defects they found.** A `dispose-concept` from a legacy page would have run the full ship pipeline (the legacy mapping is written in part letters, and inserting the implement part shifted them). A resumed close-out would have skipped the follow-ups it was told to build (the session-start mandate still said "THREE parts"). A reload during a multi-minute finalize returned a fully re-armed sheet whose second execute carries a new `submission_id` the replay guard cannot recognise — issues and a release, twice; the sheet asks the bridge what is in flight before arming anything now, and the safety net no longer re-arms a delivered finalize. A tab detour reset every chosen route back to Issue. Two points sharing a checkbox `name` shared one radio group, and a point with no name was always filed no matter what the user clicked. The ☰ FAB, new on document rounds, covered ~44px of the frozen bar's "back to the live round" button at 375px. Prose that still promised a sidebar — the picker table, both template intros, the free-template layout section — is what a regenerating Claude reads first. New jsdom suites run the reference JS instead of grepping it; the route-reset defect was found by one of them.
...
- **A design round can now ask questions about the artefact it is showing.** Until now a `design` iteration could hold exactly one kind of thing — mockups. Everything else the round needed ("tabs or a drawer?", "which of these two card densities?") had to wait for a separate `decision` iteration one round later, out of sight of the thing it was about. Comparisons had no home at all: the decision template's `comparison` variant weighs abstract options against criteria, not two concrete component ideas side by side. A `section[data-view]` is now a first-class sibling of `data-design` — fullscreen, its own entry in the table of contents and the switcher, switched exactly like a design. Two kinds ship: `decision` reuses the existing bi-state mechanics rather than inventing a second spelling for the same thing, and `comparison` is a new template whose mandatory skeleton is deliberately small (a question, two or more candidates, a verdict control, notes) with everything else — criteria matrix, weights, pros/cons, mockup fragments — explicitly optional. It offers two layouts: an option grid when the candidates differ in shape, and a criteria table when they differ in values along shared criteria, because a grid of cards forces the reader to hop between columns to compare a single row. At least one design remains mandatory; a round that is only questions is still a `decision` iteration.

- **Claude can pin a question onto the one element it is about.** Numbered pins sit on concrete elements of a screen — anchored by CSS selector via `data-anno-target`, not by hand-guessed percentages, which drift the moment the mock reflows. Each pin opens a bubble with the question, an answer field and an attachment slot, so the answer is given at the point it belongs to instead of described in prose in a general feedback box. The bubble flips to the opposite side and shifts back into view when it would open off-screen. One eye pill hides the entire layer for an untouched look at the design, and stays visible as the only remnant so the layer can be brought back. Both FABs are untouched and keep working whether the layer is shown or hidden. The whole thing is optional — a design round with nothing to ask looks exactly as it did before.

- **Every input field takes files now, not just decision comments.** Drag & drop, `Ctrl+V` and a file picker work on the feedback dock at all four levels, on annotation answers and on comparison and decision notes. Any file type is accepted, not the four image formats of before. Uploads stream the raw blob instead of inflating it by a third as base64, with per-file progress, because a 200 MB file with no feedback looks like a frozen browser. The durability contract is unchanged and now applies everywhere: the blob is written to IndexedDB *before* the network call, the local copy is kept until the server confirms, content addressing makes every retry idempotent, and unsynced files are re-sent on the next reconnect.

### Fixed

- **Any website open in the browser could read every file attached to a concept.** The bridge bound all network interfaces, `GET /recovery` and `GET /attachments/<id>` had no origin check at all, and every response carried `Access-Control-Allow-Origin: *` — so a page on an unrelated tab could enumerate the local port range, read the attachment index and download the bytes. `POST /decisions` was equally open, which let a hostile page inject a submission carrying `action: "implement"`. The server now binds loopback only, checks the origin on every data-bearing endpoint, and echoes back only an origin that validates. A request with no `Origin` header keeps working — that is how Claude's own polling reaches the bridge. Widening uploads to arbitrary file types is what turned this from a screenshot leak into a document leak.

- **Two documented guarantees were weaker than the documentation claimed.** Content addressing was not content addressing: the stored id carried the client's file extension, so identical bytes uploaded as `a.bin` and `a.dat` produced two blobs and were charged twice against the store quota. And "only raster images are served inline" was enforced against the client-supplied *extension*, not the bytes — SVG or HTML uploaded as `payload.png` came back `Content-Type: image/png`, with `nosniff` as the only thing standing in the way. Dedup is now keyed on the digest alone, and the inline decision is made from the file's magic bytes.

- **Attachments were lost to a Windows path limit, silently.** Blob filenames are 64 hex characters plus an extension; in a project checked out under a deep path the full store path crosses the 260-character legacy limit, and the upload failed with a `507` that read like a disk problem. Store writes now use the extended-length path API.
...
- **An issue routed to another repository no longer gets created in the current one.** `/setup-issue` ran a bare `gh issue create` with no `--repo` anywhere in its flow, while callers had been handing it a "target repo" for some time — `/claude-learn` files a plugin defect against the plugin source repo from inside a consumer project, and a cross-project learning against that project. `gh` used whatever repo the working directory happened to be, so the issue landed in the wrong place and returned a perfectly valid-looking URL: the failure was invisible on both ends. The skill now takes a target repo explicitly, passes it as `--repo`, warns that labels and milestones configured for the current project need not exist in the target, and — because a silent misfile is the whole problem — verifies the created issue's `owner/name` against what was asked for before reporting success. This is the same defect the previous release addressed at the routing layer, one layer down.

- **`/claude-learn`'s routing decision no longer has inputs that fit nowhere, or in two places.** The restructure that collapsed it into a single decision left real gaps, found by review before release: Q1 offered only "the plugin" or "this project", so a rule for a *named other project* had no answer and fell through to the current project's instructions — writing into the wrong repo — while a rule belonging to no project at all ("answer me in German everywhere") did the same silently. Q1 now asks plugin-or-a-project, a separate question says which project, and the no-project case is a gate that runs before Q1, since it cannot answer it. The "when unsure, treat it as a plugin rule" tie-breaker had no repo scope and inverted inside the plugin's own repo, where it would ship a dotclaude-only convention to every consumer as a default; it now applies only outside that repo. The local-override exception accepted any stated reason, including one that merely explained the local need — it now requires the reason to be why *every other project* would be wrong to inherit the rule, with worked examples of both readings. Six worked examples pin one branch each.

- **A repo-local rule in dotclaude no longer ships to every consumer as a plugin default.** The local-override case required "this session is not the plugin repo", which made it unreachable inside dotclaude — so *"`/ship` soll hier den Docker-Publish überspringen"*, captured there, routed to a direct edit of the shipped skill instead of the repo's own `.claude/`. Being inside the plugin's repo does not make a repo-local rule a plugin default: dotclaude is a consumer of its own plugin, and the condition is now about the rule, not about which repo you happen to sit in. The same round removed two more ways to land wrong: the routing question matched both of its own answers for any rule naming a plugin part with local-only reach, and the global-rule gate stopped on the phrase "in jedem Projekt" — catching *"in jedem Projekt soll `/commit` die Issue-Nummer schreiben"*, which is a plugin-default change, not a global preference.

- **The plugin-source-repo check now matches the code it claims parity with.** The skill described the detection as "plugin.json exists and names devops", while the shared implementation also accepts the root marketplace.json and — crucially — excludes anything under `~/.claude/plugins/{cache,marketplaces,repos}/`. A session opened in the marketplace clone therefore answered "yes, this is the source repo" and routed to a direct edit, which `pre.plugin.scope` then blocked on every write: the learning ended up persisted nowhere. Both the skill and the canonical hierarchy doc now state the exclusion.

- **An out-of-bounds target path is no longer silently rewritten to the current project.** Naming a project outside `~/IdeaProjects/` hit two adjacent instructions — "treat it as no hint, and ask" and "no hint → current project, no ask" — and the second won, filing another project's rule into this one. It is now a third, named outcome that always asks.

- **An upstream issue no longer fails the gate it is handed to.** `/setup-issue` requires a `**User value:**` line and rejects issues without one; the cross-project branch supplied it, the upstream-plugin branch did not — and neither did the two scope hooks, which are the highest-traffic surface. All three now specify it, with a fallback when the gate still declines, so a declined issue no longer means the learning is persisted nowhere. Three narrower gaps in the same skill: `type:*` was carved out of label verification although `gh issue create` hard-fails on a label the target repo lacks; the landing-repo check compared slugs case-sensitively, which would fail a correctly filed issue; and the project-board step had no target-repo guard, so a cross-repo issue would appear on this project's board.

- **Stale skill names in extension paths, left behind by past renames.** `/auto-usage` looked for its project extension under `.claude/skills/refresh-usage/`, and three files still pointed at `new-issue` after it became `setup-issue`. Nothing errors in this situation: the extension is simply never loaded, and the project's customization silently does not apply. A new contract test asserts every `.claude/skills/<name>/` path in the plugin names a skill that exists, behind an exemption list that requires a written reason per entry (`devops-test-plan` is a deliberately frozen consumer contract).

- **Ambiguous `deep-knowledge/` references in skills that own a sibling directory.** Once a skill has its own `deep-knowledge/`, a bare `deep-knowledge/x.md` reads as that sibling — so a reference meant for the plugin-level file of the same name points at a path that does not exist, and the reader falls back to memory instead. Eight files across six skills were affected. Plugin-level references now carry `{PLUGIN_ROOT}/`, and a test asserts that every bare reference resolves to a real sibling. Two more inaccuracies fell out of the same sweep: a `deep-knowledge/issue-rules.md` reference in `/concept` that resolved nowhere, and `[FEAT]` used as an issue-title prefix where the canonical table says `[FEATURE]` — which `/setup-issue` treats as a hard error, so the upstream-issue branch would have failed at creation time.
...
- **`/devops-ship` no longer silently drops untracked files or fails open on a flaky remote — the release-critical git paths are hardened.** A branch-wide analysis of how ship was used across recent worktrees (and every past `fix(ship)` commit) surfaced a cluster of latent hazards, now closed: (1) **`ship_release` staged untracked files through the error-swallowing `git()` wrapper with the path interpolated unquoted into a shell** — a filename with a space/metachar word-split, `git add` failed, the file was dropped, yet the commit succeeded and `includedUntracked` falsely reported it staged; staging now goes through `execFileSync` (array args, no shell) so a failed add aborts the ship instead of losing work. (2) **`ship_promote`'s remote-truth tag queries fell open on a transient network/auth failure** — a null result parsed as an empty tag list, silently skipping the monotonicity + same-version-immutability guards and letting a downgrade re-tag through; the remote reads now fail closed (new `gitArgs` execFile helper in `lib/git.js` throws rather than returning null). (`mcp-server/ship/lib/git.js`, `tools/release.js`, `tools/promote.js`; +extended tests.)
- **The post-merge deploy-verify result is no longer lost on a worktree ship.** `post-merge-watcher.js` defaulted its state dir to `<cwd>/.claude/.ship-watcher`, but on a worktree ship `cwd` is the worktree that `ship_cleanup` deletes — so `ss.ship.verify` (reading from the main repo) never found the result. The state dir now resolves to the **main repo** via the git-common-dir (`resolveStateDir`), and the ship SKILL documents the anchor. (`scripts/post-merge-watcher.js`, `skills/devops-ship/SKILL.md` Step 4b.)
- **Several smaller ship-pipeline correctness + AFK-safety gaps closed.** `ship_version_bump` now rejects non-semver input instead of emitting a `NaN.0.0` tag (`lib/version.js`); `ship_build`'s project deep-knowledge index generator was stringified instead of called (`${DK_INDEX_SCRIPT()}` in `tools/build.js`); the ship-in-progress sentinel's stale-TTL was raised above the worst-case pipeline duration and the SKILL now clears it on every `ship-blocked`/abort path (`hooks/lib/ship-sentinel.js`, SKILL sentinel-hygiene rule); the `$SHIP_LOCKOUT` decision is persisted to a `.claude/.ship-lockout` marker and re-derived at each gate so a >5-min CI wait plus context compaction can't silently re-enable `AskUserQuestion` in an AFK run; the SKILL's `allowed-tools` gains `bash`/`nohup` (the Codex gate and watcher were off-whitelist → permission-prompt hang); and ring-model doc drift in `hierarchical-merge.md` (a ship creates an alpha tag, not a GitHub Release) is corrected. Delivered via a `/devops-concept` review page (all findings reviewed + accepted). Codex review gate unavailable this ship (external 503 + usage limit) — covered instead by full-suite verification (792 tests) and added ship-scoped unit tests.
...
- **`/devops-burn-backlog` can no longer wedge mid-run instead of shipping — the three ways a walked-away run could hang are closed.** A user who invoked the backlog runner and left the PC could return to a run stuck on a modal or a silent stall rather than shipped work, through three distinct gaps. **(1) Composed ships were interactive.** The Step 4 loop calls `/devops-ship` once per issue, but ship raises `AskUserQuestion` at five gates (session-activity, an unresolvable rebase conflict, a Codex security/logic finding, the purpose-alignment high-impact case, and a major-version bump) — none aware of the burn's Post-Confirmation Lockout. In a multi-issue night an ambiguous rebase is likely (each issue merges to main, the next rebases onto it), so a single modal froze the whole queue with nobody to answer. Fix: a new `scripts/autonomous-lockout.js` sentinel (`arm`/`check`/`clear`, self-registers in `.git/info/exclude`, and a corrupt sentinel reads as *locked* so the guard fails toward non-interactive); ship gains a **Pre-Step A** that detects it and turns every gate into a deterministic non-interactive branch — BLOCK (park the issue as a `⏸ Rückfrage`, render `ship-blocked`, queue continues) for the unsafe cases, RECORD & CONTINUE (fold into `userFinalTest`) for the non-fatal one. The guard only ever *adds* safety: a normal interactive ship, or an older plugin without the script, behaves exactly as before. **(2) The presence phase had no early timeout.** The 3-minute autostart was armed only at the final gate (Step 3.4), so a user who left during milestone selection, triage, or the shutdown question hung indefinitely on the first modal. The autostart is now armed **before the first question** (Step 1a) and re-armed after each answer, with safe defaults — all open milestones, `shutdown=yes` (the only sensible default for someone who left), undecided `needs-decision`/`oversized` items parked, not guessed. Step 0.1 gains a `phase=presence` branch that resumes from the correct point (skips GitHub-writing refine + concept decisions) versus the existing `phase=gate` path. **(3) A `shutdown=no` run had no active recovery.** The external watchdog's `notify` mode only writes a stalled marker — if Claude's own inference wedged (an Anthropic API hang), the night just idled. `autonomous-watchdog.js` gains a third **`resume`** action that notifies AND attempts a guarded one-shot external relaunch of `claude` (via `Start-Process`, in the flag's project dir, gated by a one-per-run `AUTONOMOUS-RECOVERY.flag` so a repeatedly-firing task can never fork-bomb it), degrading cleanly to notify-only when `claude` is not on `PATH`; burn arms the watchdog with `action=resume` when `shutdown=no`. Also fixes three `deep-knowledge` references in the burn skill that pointed at nonexistent plugin-root paths (the files are skill-private to `devops-autonomous`/`devops-new-issue`) and excludes the composed `AUTONOMOUS-*` artifacts from git. Codex review gate was unavailable this ship (external usage limit) — covered instead by +7 unit tests (watchdog resume-mode script generation incl. the fork-bomb guard and prompt escaping; lockout arm/check/clear incl. the corrupt-sentinel fail-safe) and full-suite verification. (New `autonomous-lockout.js` + tests, `autonomous-watchdog.js` resume action + tests, `devops-ship` SKILL 0.7.0 → 0.8.0, `devops-burn-backlog` SKILL 0.1.0 → 0.2.0, `shutdown-watchdog.md` resume-mode reference; 764 tests.)
...
- **The completion card no longer serves weeks-old usage values as if they were current — the entire usage pipeline was rebuilt around the claude.ai internal API after a 5-agent adversarial investigation.** Root causes (all verified live): (1) the Desktop app never executes the `statusLine` command (anthropics/claude-code#41456 — silently ignored), so the native usage source never wrote a single snapshot on Desktop-only machines; (2) claude.ai redirects `/settings/usage` to `/new#settings/usage` since ~2026-05-31 (settings became an SPA overlay that never renders in a headless tab), killing every DOM scrape; (3) the scraper's cache fallback exits 0, which the MCP treated as a successful scrape — it advanced the delta baseline onto the stale snapshot, freezing the card at "93% +0%" against a 33-day-old reading with only a grey `cached` note. The fix: `refresh-usage-headless.js` (0.5.0→0.6.0) now fetches usage via a **cookie-authed in-page call to `GET /api/organizations/{id}/usage` + `/rate_limits`** in the logged-in scraper profile (no token, no DOM parsing, immune to UI redesigns; richer payload incl. plan tier and model-scoped weekly; DOM parse kept only as a single-grab last resort). Writes are atomic with an EPERM retry + direct-write fallback (Windows rename contention); `markCached` persists `_failureReason`; a spawn-error listener and a guarded WS `JSON.parse` close the node-exit-1 crash class.
- **The usage MCP (`dotclaude-completion` 0.5.0→0.6.0) is now honest about staleness end-to-end.** New shared policy module `mcp-server/lib/usage-freshness.js`: the delta baseline advances **only on live snapshots** (`isLiveSnapshot` — cache-served data can no longer poison deltas into a perpetual "+0%"); snapshots older than 6h render as an explicit "No current usage data — last reading …" note instead of misleading percent bars (completion card AND `get_usage` meter); the `get_usage` JSON response carries `cached`/`ageMinutes`/`stale`/`failureReason` so programmatic consumers can tell live from cache without parsing the meter string; every scraper failure mode maps to a real reason (`describeScrapeFailure` — including the previously silent execSync-timeout `status: null` and the undocumented exit 1); and the scraper path is resolved at **call time** (`pickNewestVersionScript`), so a mid-session plugin update can no longer dangle the baked path (the observed "scrape exit code 1" incident after shipping 0.107.0/0.107.1). 46 new tests (584 pass).
...
- **The usage-refresh Edge scraper no longer accumulates orphaned `msedge.exe` instances or reopens login windows on an already-logged-in profile (follow-up to #217).** PR #217 added machine-wide locks and on-screen login bounds, but three root causes survived and, with many parallel sessions, let the scraper profile grow to **14 stray CDP instances (~4 GB RAM)** observed live, while every completion card showed a false **"Scraper not logged in"** despite valid cookies. `refresh-usage-headless.js` (`@version` 0.3.0→0.4.0): **(1) PID mistracking** — `spawn()` returns Edge's short-lived *bootstrap launcher* PID, which exits within seconds when Edge re-execs into its per-`user-data-dir` singleton, so `taskkill /PID <stored> /T` silently no-ops while the real browser lingers untracked and the next cold launch orphans yet another instance. New `reapScraperInstances()` kills by matching the unique `--user-data-dir` on the **live command line** (via `Get-CimInstance`), robust against the dead PID; `taskkill` now carries a 5s `timeout` so a wedged kill can't hang the refresh under the MCP's 60s budget. **(2) Login-window respawn** — the `tasklist`/PID-based `loginWindowAlive` guard false-negatived on the same re-exec, so the login lock was reclaimed and a fresh visible window spawned every few minutes. Replaced with an **age-bounded sticky marker** (`edge-usage-login-pending.json`): at most **one** visible login window per 30 min, cleared the instant any session scrapes successfully; the marker is claimed *before* the window opens so a concurrent cold-launch can't reap it mid-startup, and cold-launch is skipped entirely while a login is pending (no second instance contends for the same profile/CDP port). **(3) False "not logged in"** — a transient page-render timeout was coerced to `notLoggedIn` and opened a visible window even on a logged-in profile; now only an explicit `/login` redirect or login UI counts as logged-out, and a render timeout falls back to cache (code 3) without a window. The run-as-main IIFE is guarded behind `require.main === module` and the pure helpers are exported, so the module imports without spawning Edge; new `refresh-usage-headless.test.js` (14 tests) pins the login-once retry window and the usage parser. SKILL `devops-refresh-usage` rules updated to document the reaper, the at-most-one-window guarantee, and the transient-failure behavior.
