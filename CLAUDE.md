...
- **"Erst reviewen" Option**: Füge als **erste Option** eine "Erst reviewen" Variante hinzu — Label: `WARTE - Erst durchlesen`, Description: `Ich pausiere — lies den Text oben in Ruhe, dann antworte.` — wenn **oberhalb der Frage relevanter Kontext steht**, der zum Beantworten nötig ist und beim Erscheinen der Frage teilweise aus dem sichtbaren Bereich gescrollt sein könnte (Erklärungen, Pläne, Diffs, Analysen). Entscheidungskriterium: *Braucht der User den Text oben, um die Frage sinnvoll zu beantworten?* Ja → WARTE-Option. Nein (einfache Frage, kein vorheriger Textblock) → weglassen.
