...
- Cron: */30 * * * *
- Recurring: true
- Prompt: (die vollständige Feedback-Loop v2.3 Anweisung):

Feedback-Loop v2.3 — führe diese 5 Schritte der Reihe nach aus:
...
4. SKILL- UND TEMPLATE-INTERNALISIERUNG: Lies die globale CLAUDE.md (~/.claude/CLAUDE.md) und identifiziere alle Verweise auf Skills und deren deep-knowledge. Dann discovere dynamisch ALLE deep-knowledge Dateien (Glob: ~/.claude/skills/**/deep-knowledge/*.md). Berechne 25% der Gesamtanzahl (aufgerundet). Lies diese Anzahl Dateien pro Zyklus, rotierend durch die sortierte Liste (Startindex = Anzahl bisheriger Loop-Durchläufe * Batch-Größe mod Gesamtanzahl). Verinnerliche Regeln und Templates. Kein Output nötig — stille Selbstkalibrierung.
