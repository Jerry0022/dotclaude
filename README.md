...
- **<!--devops:count:hooks-->63<!--/devops:count:hooks--> Hooks** — automated guards and triggers across the full session lifecycle
...
<!--devops:count:hooks-->63<!--/devops:count:hooks--> hooks fire automatically across the session lifecycle — no user action needed.
...
- `post.flow.completion` — Keeps the completion-card contract in Claude's context: on the FIRST tool call of eve…
...
- `post.design.remind` — Once per context, when a UI file is written or edited, reminds Claude of the standing…
...
├── hooks/                         ← <!--devops:count:hooks-->63<!--/devops:count:hooks--> hooks (JS) registered in hooks.json
